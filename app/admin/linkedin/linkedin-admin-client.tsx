...
type PostStatus = "brouillon" | "publie";
...
function formatPostForCopy(post: LinkedinPost) {
  return [post.texte, post.hashtags.join(" ")].filter(Boolean).join("\n\n");
}

export function LinkedinAdminClient({ posts }: { posts: LinkedinPost[] }) {
  const [statusFilter, setStatusFilter] = useState<"tous" | PostStatus>("brouillon");
  const [copiedPostId, setCopiedPostId] = useState<string | null>(null);
  const [postStatuses, setPostStatuses] = useState<Record<string, PostStatus>>({});

  useEffect(() => {
    const initialStatuses = Object.fromEntries(posts.map((post) => [post.id, post.statut]));

    try {
      const storedStatuses = window.localStorage.getItem(statusStorageKey);
      setPostStatuses(storedStatuses ? { ...initialStatuses, ...JSON.parse(storedStatuses) } : initialStatuses);
    } catch {
  ...
  function publishPost(post: LinkedinPost) {
    const nextStatuses = {
      ...postStatuses,
      [post.id]: "publie" as PostStatus
    };

    setPostStatuses(nextStatuses);
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-ink/70">
              Affinez la liste par statut pour préparer vos prochaines publications.
            </p>
          </div>
          <div className="grid gap-3 lg:min-w-[14rem]">
            <label className="grid gap-2 text-sm font-bold text-ink" htmlFor="linkedin-status">
              ...
              <select
                ...
                value={statusFilter}
                onChange={(event) => setStatusFilter(event.target.value as "tous" | PostStatus)}
              >
            ...
            return (
              <article key={post.id} className="premium-card rounded-lg p-6 pl-8 text-ink sm:p-7 sm:pl-9">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <span className={`inline-flex w-fit rounded-full border px-3 py-1 text-sm font-bold ${statusClasses[currentStatus]}`}>
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <button
                    type="button"
                    className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-ink px-5 py-3 text-sm font-bold text-white shadow-card transition hover:-translate-y-0.5 hover:bg-primary focus-visible:outline-ess"
                  ...
                  {currentStatus === "brouillon" ? (
                    <button
                      type="button"
                      className="inline-flex min-h-11 items-center justify-center rounded-md border border-secondary/30 bg-secondary/10 px-5 py-3 text-sm font-bold text-ink transition hover:-translate-y-0.5 hover:border-secondary hover:bg-secondary/20 focus-visible:outline-ess"
